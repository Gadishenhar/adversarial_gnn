...
file_dir = os.path.dirname("./helper.py")
sys.path.append(file_dir)
file_dir2 = os.path.dirname("./neighbor_attack.py")
sys.path.append(file_dir2)
from GAL.helper import *
...
def parse_args():
    ...
    if args.task == 'Node_Attack':
        if args.model == "Fixed_Embedding": from Movielens.node_attack_fixed import run
        elif args.model == "Naive_GNN": 
            print("Ignoring lambda_reg field and setting it to 0.")
            args.lambda_reg=0
            from GAL.node_attack import run
        elif args.model == "GAL": 
            from GAL.node_attack import run
        ...
        if args.model == "Fixed_Embedding": from Movielens.neighbor_attack_fixed import run
        elif args.model == "Fixed_Embedding_NoTrain": from Movielens.neighbor_attack_fixed import run
        elif args.model == "Naive_GNN": 
            print("Ignoring lambda_reg field and setting it to 0.")
            args.lambda_reg=0
            from GAL.Movielens.neighbor_attack import run
        elif args.model == "GAL": 
            from GAL.neighbor_attack import run
        else: run = fail_continuation

    elif args.task == 'NHop_Attack':
        from Movielens.n_hop import run
